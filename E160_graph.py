class E160_graph:
	...
	def __init__ (self):
		...
		c1 = scale*.25*(pow(5, .5) - 1)
		c2 = scale*.25*(pow(5, .5) + 1)
		s1 = scale*.25*pow((10 + 2*pow(5, .5)), .5)
		s2 = scale*.25*pow((10 - 2*pow(5, .5)), .5)
	...
	def update_occupied_nodes(self, robots):
		self.occupied_nodes = []

		for r in robots:
			self.occupied_nodes.append(r.node_des)
